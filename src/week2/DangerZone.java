...
public class DangerZone {
	...
	public static void listOptions(){
		...
		try {
			...
			System.err.println("  Input must be a whole number between 1 and 9.");
			System.out.println();
			input = scanner.nextInt();
			scanner.close();
		}
		//even if there's an exception, line 53 doesn't run. Is there
		//no point to catching exceptions other than tracing problems and
		//making code compile? Can you ever catch an exception and keep
		//going to the next line of code?
	...
	public static void sellJet() {
		...
		Pilot p = fleet[input - 1].getPilot();
		biggerBarracks(p);

		Pilot[] smallerAssigned = new Pilot[assigned.length - 1];

		int j = 0;
		for (int i = 0; i < assigned.length; i++) {
			if (assigned[i] != p) {
				smallerAssigned[j] = assigned[i];
				j++;
			}
		}

		assigned = smallerAssigned;

		fleet[input - 1] = n;
		int q = 0;
		for (int i = 0; i < fleet.length; i++) {
			if (fleet[i] != n) {
				smallerFleet[q] = fleet[i];
				q++;
			}
		}
	...
	public static void hirePilot() {
		...
		Pilot p = new Pilot(name, rank, age);
		biggerBarracks(p);
	}
	...
	public static void firePilot() {
		...
		listPilots();
		System.out.print("CHOOSE PILOT TO FIRE: ");
